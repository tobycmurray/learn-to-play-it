...
# 3. Pre-release smoke test (see "Pre-release test procedure" below).

# 4. Tag the commit and create the GitHub release.
packaging/macos/publish_release.sh

# 5. Bump pyproject.toml to the next planned version (e.g. 0.2.0 → 0.3.0), commit, push.
```
...
## Homebrew dylibs are stripped from the bundle

PyInstaller's analysis opportunistically picks up dylibs from `/opt/homebrew`
...
analyser. The bundled binaries don't have *required* references to these libs;
empirically, builds done with `/opt/homebrew` moved aside don't include them
and the .app works end-to-end.

But Homebrew dylibs are built against the host SDK (a recent macOS), so a
single included codec lib (libvmaf, libx264, libx265, libdav1d, libvpx)
elevates the .app's `LSMinimumSystemVersion` via `patch_info_plist.py` — we've
seen minos jump from macOS 14 to macOS 26 over a single leaked
`libvmaf.3.dylib`.

The spec file (`learn-to-play-it.spec`) strips any binary under
`/opt/homebrew/`, `/usr/local/Cellar/`, or `/usr/local/opt/` from `a.binaries`
after Analysis, before COLLECT. The build log will print exactly which dylibs
were stripped, so any regression in PyInstaller's discovery behaviour is
visible. No `sudo mv` dance is needed; the .app stays Homebrew-free regardless
of the host's state.

This trades opportunistic-but-unused dylibs for a stable low minos. If a future
change ever genuinely needs a Homebrew-sourced binary, the strip would silently
break runtime loading — the smoke test below would catch it.
