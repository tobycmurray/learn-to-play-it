...
| 1.5 | Press 0 | Restarts from 0.0s. Audio plays again |
| 1.6 | Press Q | Player exits cleanly, terminal restored to normal |
...
| 2.2 | Press X twice | Speed shows 30%. No audio (paused) |
| 2.3 | Press SPACE to resume | Audio plays at 30% speed (noticeably slower, pitch unchanged) |
| 2.4 | Press W three times | Speed shows 60%. Brief silence during each re-stretch (~2s), then resumes. Position doesn't jump back |
...
| Step | Action | Expected |
|------|--------|----------|
| 3.1 | Press E | Pitch shows +10c. Brief silence then audio resumes pitched slightly up |
...
| 6.7 | Press [ | Loop cleared, new start set at current position. Status shows new start and `?` for end |

## 7. Loop + seek interaction
...
| 7.5 | Press 0 while loop is active | Position jumps to loop start (20s), not song start |
| 7.6 | Press L to disable loop, then press 0 | Position jumps to song start (0.0s) |
...
| Step | Action | Expected |
|------|--------|----------|
| 9.1 | Play until an interesting note, press H | Status shows `⏺`. A short slice of audio repeats continuously |
| 9.2 | Press E or C while holding | Pitch changes audibly on the held note (brief silence during rebuild) |
| 9.3 | Press W or X while holding | Speed changes audibly on the held note |
| 9.4 | Press H again | Hold released. Playback resumes from where hold was activated. Status shows `▶` |

## 10. play-along command

Launch with: `ltpi play-along /tmp/safm.mp3 guitar`

| Step | Action | Expected |
|------|--------|----------|
| 10.1 | Launch | Audio plays at 100% speed in mute mode (everything except guitar). Status shows speed 100%, mode mute |
| 10.2 | All other controls | Work the same as in practice mode |

## 11. Separation and caching

| Step | Action | Expected |
|------|--------|----------|
| 11.1 | `ltpi separate /tmp/safm.mp3` (already separated) | Prints "Stems already exist", lists 6 stems |
| 11.2 | `ltpi parts /tmp/safm.mp3` | Lists: vocals, drums, bass, guitar, piano, other |
| 11.3 | `rm -rf stems/safm && ltpi practice /tmp/safm.mp3 bass` | Auto-separates (takes a few minutes), then enters player |

## 12. Error cases

| Step | Action | Expected |
|------|--------|----------|
| 12.1 | `ltpi separate nonexistent.mp3` | Error message about file not found (from Click) |
| 12.2 | `ltpi practice /tmp/safm.mp3 banjo` | Error message about invalid choice (from Click) |
